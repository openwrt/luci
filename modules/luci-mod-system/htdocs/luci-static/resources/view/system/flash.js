	...
	handleSysupgrade(storage_size, has_rootfs_data, ev) {
		return ui.uploadFile('/tmp/firmware.bin', ev.target.firstChild)
			...
			.then(L.bind(function(btn, res) {
				...
				body.push(E('p', {}, E('label', { 'class': 'cbi-checkbox' }, [
					opts.keep[0], ' ',
				...
				if (!allow_backup) {
					...
					if (has_rootfs_data) {
						body.push(E('p', {}, E('label', { 'class': 'cbi-checkbox' }, [
							...
							_('Skip from backup files that are equal to those in /rom')
						])));
					}
					...
					body.push(E('p', {}, E('label', { 'class': 'cbi-checkbox' }, [
						...
						_('Include in backup a list of current installed packages at /etc/backup/installed_packages.txt')
					])));
				};
				...
				if ((!is_valid || is_too_big || res[2].code != 0) && is_forceable) {
					body.push(E('p', {}, E('label', { 'class': 'cbi-checkbox alert-message danger' }, [
						opts.force[0], ' ',
