...
return network.registerProtocol('wireguard', {
	...
	renderFormOptions: function(s) {
		...
		o.createPeerConfig = function(section_id, endpoint, ips, dns) {
			var pub = s.formvalue(s.section, 'public_key'),
		...
		o.handleGenerateQR = function(section_id, ev) {
			...
			return Promise.all([
				...
				qrm = new form.JSONMap({ config: { endpoint: hostnames[0], allowed_ips: ips, dns_servers: dns } }, null, _('The generated configuration can be imported into a WireGuard client application to set up a connection towards this device.'));
				qrm.parent = parent;
				...
				function handleConfigChange(ev, section_id, value) {
					...
					if (this.isValid(section_id)) {
						conf.firstChild.data = configGenerator(endpoint.getValue(), ips.getValue(), dns.getValue());
				...
				qro.renderWidget = function() {
					var peer_config = configGenerator(hostnames[0], ips, dns);
