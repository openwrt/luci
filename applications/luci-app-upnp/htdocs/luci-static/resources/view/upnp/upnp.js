...
return view.extend({
	...
	render: function(data) {
		...
		s.tab('access_control', _('Access Control'), '<h5>' + _('Access Control Defaults') + '</h5>' +
			_('Use an access preset for ports that all devices can map.') + ' ' +
			_('Alternatively, add client-specific permissions using the access control list (ACL), which can also extend/override the preset.') + ' ' +
		...
		o.value('', _('None / only accept listed ports'));
		...
		o = s.taboption('access_control', form.Flag, 'check_acl', _('Check ACL'),
			_('Check ACL entries before the preset; can extend/override the preset') + '<br />' +
		...
		s = m.section(form.GridSection, 'acl_entry', '<h5>' + _('Access Control List') + '</h5>',
			...
			_('ACL entries are checked in order and rejected by default, with no preset. (should be part of access control tab)'));
		s.anonymous = true;
		s.addremove = true;
		s.cloneable = true;
		s.sortable = true;
