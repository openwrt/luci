...
return view.extend({
	...
	render: function(data) {
		...
		m = new form.Map('upnpd', [_('UPnP IGD & PCP/NAT-PMP Service')],
			_('The %s protocols enable devices on the local network to autonomously configure port maps/forwards on the router.',
				'The %s (%s = UPnP IGD & PCP/NAT-PMP) protocols enable devices on the local network to autonomously configure port maps/forwards on the router.')
				.format(protocols)
		);
		...
		s.render = L.bind(function(view, section_id) {
			var table = E('table', { 'class': 'table cbi-section-table', 'id': 'upnp_status_table' }, [
				E('tr', { 'class': 'tr table-titles' }, [
					...
					E('th', { 'class': 'th' }, _('Opened Via') + ' / ' + _('Description')),
		...
		o = s.taboption('advanced', form.RichListValue, 'use_stun', _('Enable %s', 'Enable %s (%s = CGNAT)')
			...
			_('Enables use of unrestricted endpoint-independent (1:1) CGNATs and detects the public IPv4'));
		o.value('', _('Disabled'), _('Override public IPv4 to allow private IPv4 on WAN'));
		...
		s = m.section(form.GridSection, 'internal_network', '<h5>' + _('Enabled Networks / Access Control List') + '</h5>',
			_('Select local/internal networks to enable service for. The ACL specifies which client addresses and ports can be mapped.') + ' ' +
			_('IPv6 is always accepted unless disabled.'));
		s.anonymous = true;
		s.addremove = true;
		...
		o = s.option(form.ListValue, 'check_custom_acl', '',
			_('Whether the custom ACL entries should also be checked/processed'));
