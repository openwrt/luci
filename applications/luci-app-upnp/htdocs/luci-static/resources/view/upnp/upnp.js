...
return view.extend({
	...
	render: function(data) {
		...
		if (!uci.get('upnpd', 'config', 'enabled')) {
			ui.addNotification(null, E('p', _('No suitable config file found. Please update both packages (daemon and LuCI) and restart service.') + ' (v1.0)'));
			return;
		}
		...
		s = m.section(form.GridSection, 'internal_network', '<h5>' + _('Enabled Networks / Access Control List Preset') + '</h5>',
			_('Choose local/internal networks to enable the service for. An ACL preset specifies which ports all devices on the network can map, or use a custom/client-specific ACL.') + ' ' +
		...
		o.value('', _('None / check custom ACL only'));
		...
		o = s.option(form.ListValue, 'acl_check_custom', '',
			_('Whether the custom ACL entries should additionally be checked'));
		o.value('', _('Ignore custom ACL'));
		o.value('first', _('Check custom ACL first'));
		o.value('last', _('Check custom ACL last'));
		o.editable = true;
		o.depends({ acl_preset: '', '!reverse': true });
		...
		o = s.option(form.Value, 'acl_reject_unsafe_ports', _('Reject unsafe/insecure/risky ports'),
			_('By default, reject FTP/Telnet/DCE/NetBIOS/SMB/RDP client ports being mapped regardless of the ACL'));
		o.placeholder = '21 23 135 137-139 445 3389';

		s = m.section(form.GridSection, 'perm_rule', '<h5>' + _('Custom ACL') + '</h5>',
			_('The ACL specifies which client addresses and ports can be mapped. ACL entries are checked in order, and then rejected by default.') + ' ' + _('An empty list is reset to defaults when applied. (alternative text welcome)'));
		s.anonymous = true;
		s.sortable = true;
		s.addremove = true;
		s.cloneable = true;
		...
		for (let ifnr = 0; uci.get('upnpd', `@internal_network[${ifnr}]`, 'interface'); ifnr++) {
			if (!uci.get('upnpd', `@internal_network[${ifnr}]`, 'acl_preset') ||
				uci.get('upnpd', `@internal_network[${ifnr}]`, 'acl_check_custom')) {
				custom_acl = true;
			}
		}
		...
		o = s.option(form.Value, 'desc_filter', _('Description Filter'),
			_('A regular expression to check for the UPnP IGD IPv4 port map description'));
