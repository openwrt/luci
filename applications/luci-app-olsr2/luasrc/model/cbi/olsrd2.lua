...
if not arg[1] or m.uci:get("olsrd2", arg[1]) ~= "interface" then
	...
	svc.datatype = "string"
	--svc.datatype = "or(ip6prefix, ip4prefix, 'default_accept', 'default_reject')"
	svc.optional = true
	...
	svc.datatype = "string"
	--svc.datatype = "or(ip6prefix, ip4prefix)"
	svc.optional = true
	...
	local ign = ifs:option(Flag, "ignore", translate("Enable"))
	ign.enabled  = "0"
	ign.disabled = "1"
	ign.rmempty = false
	...
	local svc = ifs:option(Value, "ifname", translate("Network"),
	...
	ifs = m:section(NamedSection, arg[1], "interface", translate("interface"))
	ifs.anonymous = true
	ifs.addremove = false
	local svc = ifs:option(DynamicList, "acl", translate("acl defines the IP addresses that are allowed to use the RFC5444 socket."), "ip6prefix, ip4prefix, default_accept, default_reject")
	svc.datatype = "string"
	--svc.datatype = "or(ip6prefix, ip4prefix, 'default_accept', 'default_reject')"
	svc.optional = true
	local svc = ifs:option(DynamicList, "bindto", translate("bindto defines the IP addresses which the RFC5444 socket will be bound to."), "ip6prefix, ip4prefix, default_accept, default_reject")
	svc.datatype = "string"
	--svc.datatype = "or(ip6prefix, ip4prefix, 'default_accept', 'default_reject')"
	svc.optional = true
	local svc = ifs:option(Value, "multicast_v4", translate("multicast_v4 defines the IPv4 multicast address used for RFC5444 packets."), "ip4addr")
	svc.datatype = "ip4addr"
	svc.placeholder = "224.0.0.109"
	svc.optional = true
	local svc = ifs:option(Value, "multicast_v6", translate("multicast_v6 defines the IPv4 multicast address used for RFC5444 packets."), "ip4addr")
	svc.datatype = "ip6addr"
	svc.placeholder = "ff02::6d"
	svc.optional = true
	local svc = ifs:option(Value, "dscp", translate("dscp defines the DSCP value set for each outgoing RFC5444 packet. The value must be between 0 and 252 without fractional digits. The value should be a multiple of 4."), "0-255")
	svc.optional    = true
	svc.placeholder = 192
	svc.datatype    = "range(0,255)"
	local svc = ifs:option(Flag, "rawip", translate("rawip defines if the interface should put RFC5444 packets directly into IP headers (skipping the UDP header)."))
	svc.optional = true
	local svc = ifs:option(DynamicList, "ifaddr_filter", translate("ifaddr_filter defines the IP addresses that are allowed to NHDP interface addresses."), "ip6prefix, ip4prefix, default_accept, default_reject")
	svc.datatype = "string"
	--svc.datatype = "or(ip6prefix, ip4prefix, 'default_accept', 'default_reject')"
	svc.optional = true
	local svc = ifs:option(Value, "hello_validity", translate("hello_validity defines the time the local HELLO messages will be valid for the neighbors."), ">0.1 s")
	svc.optional    = true
	svc.placeholder = 20.0
	svc.datatype    = "and(min(0.1), ufloat)"
	local svc = ifs:option(Value, "hello_interval", translate("hello_interval defines the time between two HELLO messages on the interface."), ">0.1 s")
	svc.optional = true
	svc.placeholder = 2.0
	svc.datatype = "and(min(0.1), ufloat)"
	local svc = ifs:option(Value, "rx_bitrate", translate("rx_bitrate"))
	svc.optional = true
	svc.rmempty = false
	svc.placeholder = "1G"
	svc.datatype = "string"
	local svc = ifs:option(Value, "tx_bitrate", translate("tx_bitrate"))
	svc.optional = true
	svc.rmempty = false
	svc.placeholder = "1G"
	svc.datatype = "string"
	local svc = ifs:option(Value, "rx_max_bitrate", translate("rx_max_bitrate"))
	svc.optional = true
	svc.rmempty = false
	svc.placeholder = "1G"
	svc.datatype = "string"
	local svc = ifs:option(Value, "tx_max_bitrate", translate("tx_max_bitrate"))
	svc.optional = true
	svc.rmempty = false
	svc.placeholder = "1G"
	svc.datatype = "string"
	local svc = ifs:option(Value, "rx_signal", translate("rx_signal"))
	svc.optional = true
	svc.rmempty = false
	svc.placeholder = "1G"
	svc.datatype = "string"
