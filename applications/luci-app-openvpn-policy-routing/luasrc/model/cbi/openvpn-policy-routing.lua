...
-- Domains
if luci.sys.call("dnsmasq -version | grep -q ipset") == 0 and nixio.fs.access("/var/run/dnsmasq/") and nixio.fs.access("/etc/config/dhcp") then
  for f in nixio.fs.dir("/var/run/dnsmasq/") do
    if string.match(f, "dnsmasq.*.pid") then
...
if dnsserver == "dnsmasq" then
  d = Map("dhcp")
  s2 = d:section(TypedSection, "dnsmasq", translate("Domain-based Policies (dnsmasq)"))
  s2.anonymous = true
  ...
  s2 = d:section(TypedSection, "domain-policy", translate("Domain-based Policies (internal)"))
  s2.anonymous = true
  s2.addremove = true
  di = s2:option(DynamicList, "ipset", translate("Domain Policies"), translate("Domains routed via specific gateways, see ")
    .. [[<a href="]] .. readmeURL .. [[#domain-based-policies" target="_blank">]]
