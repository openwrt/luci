...
function parseApkQueryJson(s, dest) {
	...
	for (const item of rawData) {
		...
		// Define the package object (merging name back in)
		const pkg = { name, ...attributes };
...
function updateLists(data)
{
	...
	return (data ? Promise.resolve(data) : downloadLists()).then(function(data) {
		...
		parseApkQueryJson(data[1], packages.available);
		parseApkQueryJson(data[2], packages.installed);
