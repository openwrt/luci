...
return view.extend({
	load() {
		return Promise.all([
			...
			L.resolveDefault(fs.read_direct('/usr/share/acmesh/dnsapi.info.txt'), ''),
		]);
	},
	...
	render(data) {
		...
		for (let info of apiInfos) {
			for (let opt of info.Opts) {
				_addDnsProviderField(s, info.Id, opt);
			}
...
function _addDnsProviderField(s, apiId, opt) {
	let desc = '<code>' + opt.Name + '</code> ' + opt.Description;
	if (opt.Default) {
		desc += '<br />' + _('Default')  + ' <code>' + opt.Default + '</code>';
	}
	let o = s.taboption('challenge_dns', form.Value, '_credentials_' + opt.Name, opt.Title, desc);
	o.depends('dns', apiId);
	o.modalonly = true;
