...
return baseclass.extend({
	...
	rrdargs: function(graph, host, plugin, plugin_instance, dtype) {
		...
		var rv = [];
		var instances;
		var typeinstances = graph.dataInstances(host, plugin, plugin_instance, "if_octets").sort();
		...
		if ((instances = find_instances('if_octets', ['overhead', 'traffic'])) !== null) {
			...
			for (let i = 0; i < typeinstances.length; i++) {
				var vpnuser = typeinstances[i];
				var rx = "if_octets_%s_rx".format(vpnuser),
					tx = "if_octets_%s_tx".format(vpnuser),
					opts = {};
				opts[tx] = { weight: 0, title: "Bytes " + vpnuser + "   (TX)", total: true, color: "00ff00" };
				opts[rx] = { weight: 1, title: "Bytes " + vpnuser + "   (RX)", total: true, flip: true, color: "0000ff" };

				rv.push({
					title: "%%H: OpenVPN Server \"%s\" - User Traffic - ".format(inst) + vpnuser,
					vlabel: "Bytes/s",
					data: {
						instances: {
							if_octets: [vpnuser]
