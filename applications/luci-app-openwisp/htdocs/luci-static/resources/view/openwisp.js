...
return view.extend({
	render: function () {
		...
		m = new form.Map('openwisp', 
			_('OpenWISP'), 
			_("Configure, start and stop the OpenWISP agent on this device. Read more about configuration values: <a href='https://github.com/openwisp/openwisp-config'>https://github.com/openwisp/openwisp-config</a>"));
		...
		o = s.taboption('advanced', form.Flag, 'consistent_key', _('Consistent Key'), _('When using Automatic registration, this feature allows devices to keep the same configuration even if reset or reflashed.  This feature is enabled by default, but must be enabled also in the controller application in order to work.'))
		o.rmempty = false
		...
		o = s.taboption('advanced', form.Flag, 'merge_config', _('Merge Config'), _('If selected, in the event a config item is present in both the remote and local configuration, the remote configuration takes prescedence over local configuration.'))
		o.rmempty = false
		...
		o = s.taboption('advanced', form.Value, 'bootup_delay', _('Bootup Delay'), 'Maximum value of the bootup delay before starting OpenWISP service. Expressed in seconds.')
		o.placeholder = '0'
		o.datatype = 'uinteger';
