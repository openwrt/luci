...
local knownParams = {
	...
	{ "VPN", {
		...
		{ Value,		"server_bridge",				"10.200.200.1 255.255.255.0 10.200.200.200 10.200.200.250",		translate("Configure server bridge") },
		{ DynamicList,	"push",							{ "redirect-gateway", "comp-lzo" },								translate("Push options to peer") },
		{ Flag,			"push_reset",					0,																translate("Don't inherit global push options") },
		{ Flag,			"disable",						0,																translate("Client is disabled") },
		{ Value,		"ifconfig_pool",				"10.200.200.100 10.200.200.150 255.255.255.0",					translate("Set aside a pool of subnets") },
		{ Value,		"ifconfig_pool_persist",		"/etc/openvpn/ipp.txt 600",										translate("Persist/unpersist ifconfig-pool") },
--		{ Flag,			"ifconfig_pool_linear",			0,																translate("Use individual addresses rather than /30 subnets") }, -- deprecated and replaced by --topology p2p
		{ Value,		"ifconfig_push",				"10.200.200.1 255.255.255.255",									translate("Push an ifconfig option to remote") },
		{ Value,		"iroute",						"10.200.200.0 255.255.255.0",									translate("Route subnet to client") },
		{ Flag,			"client_to_client",				0,																translate("Allow client-to-client traffic") },
		{ Flag,			"duplicate_cn",					0,																translate("Allow multiple clients with same certificate") },
		{ Value,		"client_config_dir",			"/etc/openvpn/ccd",												translate("Directory for custom client config files") },
		{ Flag,			"ccd_exclusive",				0,																translate("Refuse connection if no custom client config") },
		{ Value,		"tmp_dir",						"/var/run/openvpn",												translate("Temporary directory for client-connect return file") },
		{ Value,		"hash_size",					"256 256",														translate("Set size of real and virtual address hash tables") },
		{ Value,		"bcast_buffers",				256,															translate("Number of allocated broadcast buffers") },
		{ Value,		"tcp_queue_limit",				64,																translate("Maximum number of queued TCP output packets") },
		{ Value,		"max_clients",					10,																translate("Allowed maximum of connected clients") },
		{ Value,		"max_routes_per_client",		256,															translate("Allowed maximum of internal") },
		{ Value,		"connect_freq",					"3 10",															translate("Allowed maximum of new connections") },
		{ Flag,			"client_cert_not_required",		0,																translate("Don't require client certificate") },
		{ Flag,			"username_as_common_name",		0,																translate("Use username as common name") },
