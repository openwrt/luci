...
return view.extend({
	...
	renderNats: function(data) {
		...
		have_fw4 = L.hasSystemFeature('firewall4');
		...
		o = fwtool.addIPOption(s, 'general', 'src_ip', _('Source address'),
			_('Match forwarded traffic from this IP or range.'), !have_fw4?'ipv4':'', hosts);
		o.rmempty = true;
		o.datatype = !have_fw4?'ip4addr("nomask")':'ipaddr("nomask")';
		...
		o = fwtool.addIPOption(s, 'general', 'dest_ip', _('Destination address'),
			_('Match forwarded traffic directed at the given IP address.'), !have_fw4?'ipv4':'', hosts);
		o.rmempty = true;
		o.datatype = !have_fw4?'ip4addr("nomask")':'ipaddr("nomask")';
