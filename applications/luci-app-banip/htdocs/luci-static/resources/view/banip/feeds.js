...
return view.extend({
	...
	render: function (data) {
		...
		for (let i = 0; i < Object.keys(m.data.data).length; i++) {
			...
			o.value('\/^.*\\\/\\d{2}\/{print $0; next};match($0,\/^([0-9A-f]{1,4}:){4}\/) { print substr($0, RSTART, RLENGTH)\":\/64,\";next };match($0,\/^([0-9A-f]{1,4}:){3}:\/) { print substr($0, RSTART, RLENGTH)\"\/48,\";next };\/(([0-9a-fA-F]{0,4}:){1,7}[0-9a-fA-F]{0,4})\/{print $1\",\"}', _('Rewrite IPV6 without CIDR subnet, using a double colon hint, to /64 or /48 subnets'));
			o.optional = true;
			o.rmempty = true;
