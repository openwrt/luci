...
const methods = {
	...
	get_version: {
		call: function() {
			let hbbs_version = null;
			let hbbr_version = null;

			if (fileExists(BIN_DIR + '/hbbs')) {
				hbbs_version = execCommand(BIN_DIR + '/hbbs --version 2>&1');
			}

			if (fileExists(BIN_DIR + '/hbbr')) {
				hbbr_version = execCommand(BIN_DIR + '/hbbr --version 2>&1');
			}

			return {
				hbbs_version: hbbs_version,
				hbbr_version: hbbr_version
			};
		}
	},
