...
function validateKey(section_id, value) {
	...
	if (/[^A-Za-z0-9+/=]/.test(value))
		return _('Invalid characters. Only alphanumeric and base64 characters (+/=) allowed');
	return true;
}
				...
				E('div', { 'class': 'cbi-value' }, [
					...
					E('div', { 'class': 'cbi-value-field', 'style': 'display: flex; align-items: center; flex-wrap: wrap; gap: 8px;' }, [
						...
						E('button', {
							...
							'click': function () {
								if (!keyExistsGlobal) {
									ui.addNotification(null, E('p', _('Cannot regenerate: No public key exists yet. Start the service first to generate the initial key.')), 'warning');
									return;
								}
								if (!confirm(_('This will regenerate the key pair and restart the service. All existing clients will need to be reconfigured. Continue?'))) {
									return;
								}
								this.disabled = true;
								this.textContent = _('Regenerating...');

								L.resolveDefault(callRegenerateKey(), {}).then(function (res) {
									if (res && res.success) {
										ui.addNotification(null, E('p', _('Keys deleted. Starting service to generate new keys...')), 'notice');
										// Use RPC to start service for reliable execution
										// Add small delay to ensure service has fully stopped
										return new Promise(function (resolve) {
											setTimeout(resolve, 1000);
										}).then(function () {
											return L.resolveDefault(callServiceAction('start'), {});
										});
									} else {
										ui.addNotification(null, E('p', _('Key regeneration failed: ') + (res.message || 'Could not delete keys')), 'error');
										throw new Error('Regeneration failed');
									}
								}.bind(this)).then(function (startRes) {
									if (startRes && startRes.success) {
										ui.addNotification(null, E('p', _('Service started with new key')), 'notice');
									} else if (startRes) {
										ui.addNotification(null, E('p', _('Service start may have failed. Check status above.')), 'warning');
									}
								}).catch(function (err) {
					...
					E('div', { 'class': 'cbi-value-field' }, [
						E('div', { 'style': 'margin-bottom: 8px;' }, [
							E('button', {
								...
								'click': function () {
									if (!anyServerEnabledGlobal) {
										ui.addNotification(null, E('p', _('Cannot start service: The ID Server or Relay Server must be enabled in the configuration first. Please check "Enable ID Server" or "Enable Relay Server" below and click "Save & Apply".')), 'error');
										return;
									}

									this.disabled = true;
									handleAction('start').then(function () {
										this.disabled = !anyServerEnabledGlobal;
									}.bind(this)).catch(function (err) {
										ui.addNotification(null, E('p', _('Failed to start service: ') + err.message), 'error');
										this.disabled = !anyServerEnabledGlobal;
									}.bind(this));
								}
							}, _('Start')),
							...
							E('button', {
								...
								'click': function () {
									this.disabled = true;
									handleAction('stop').then(function () {
										this.disabled = false;
									}.bind(this)).catch(function (err) {
										ui.addNotification(null, E('p', _('Failed to stop service: ') + err.message), 'error');
										this.disabled = false;
									}.bind(this));
								}
							}, _('Stop')),
							...
							E('button', {
								...
								'click': function () {
									this.disabled = true;
									handleAction('restart').then(function () {
										this.disabled = false;
									}.bind(this)).catch(function (err) {
		...
		*/
		poll.add(function () {
			return Promise.all([
				...
				const keyInfo = results[1] || {};
				const verInfo = results[2] || {};
