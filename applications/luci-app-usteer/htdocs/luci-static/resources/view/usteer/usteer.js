...
var Settingstitle = form.DummyValue.extend({
	renderWidget: function() {
		var body = E([
			E('h3', 'Settings'),
			E('div','First four options are mandatory. Also be sure to enable rrm reports, 80211kv, etc see: https://openwrt.org/docs/guide-user/network/wifi/usteer'),
		]);
		return E('div', [body]);
	}
});
...
return view.extend({
	...
	render: function(data) {
		...
		if (!('usteer' in data[0])) {
			m = new form.Map('usteer', _('Usteer'),_('usteer not running. make sure it is installed and running. Try /etc/init.d/usteer start'));
			return m.render();
		}
